...
derr = -0.00;%0.6-sdist; % error margin, to avoid overshoot and oscilation

v = 0; % begin speed
a165 = 1.23; % acceleration with motor at full power
a135 = -6.45; % acceleration with motor full power backwards while moving forwards
...
%%% distance
tmp = linspace(0.1,0.1,40); % add 0.05 for short begin dist and substr 0.05 for long begin dist
...
while 1
	...
	r = r+1;
	if r > r_val
		if min(sensors()/100) < 1.3+dmar % currently the minimal sensor value is used mean is another option
			break;
		end
		r = 0;
	end
